...
class PlayState extends MusicBeatState
{
	...
	public static var extraHUD:FlxCamera;
	...
	override public function create()
	{
		...
		extraHUD = new FlxCamera();
		extraHUD.bgColor.alpha = 0;
		FlxG.cameras.add(extraHUD, false);
		...
		if (!skipCutscenes())
			...
			// var blackFuck = new FlxSprite(0, 0).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
			// add(blackFuck);
			...
			// var startText = new FlxSprite(0, 0).loadGraphic(Paths.image('startScreen/' + SONG.song.toUpperCase() + "Text"));
			...
			// 	FlxTween.tween(assText, {alpha: 0}, 1);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.pressed.U){
			Application.current.window.width += Std.int(elapsed * 900);
		}
		if (FlxG.keys.justPressed.Y){
			Application.current.window.alert("MORISTE, AHORA TENDRE TU PC", "Sonic.exe: ");
		}
		...
		if (!inCutscene)
		{
			...
			if (!isStoryMode)
			{
				...
				if (FlxG.keys.justPressed.EIGHT){
					Main.switchState(this, new meta.state.menus.AnimationDebug(SONG.player2));
				}
				if (FlxG.keys.justPressed.NINE){
					Main.switchState(this, new meta.state.menus.AnimationDebug(SONG.player1));
				}
	...
	function characterPlayAnimation(coolNote:Note, character:Character)
	{
		...
		// 	case 1:
		// 		Lib.application.window.move(0, 45);
		// 	case 2:
		// 		Lib.application.window.move(0, -45);
		// 	case 3:
		// 		Lib.application.window.move(45, 0);
	...
	function endSong():Void
	{
		...
		else
		{
			...
			{
				// play menu music
				ForeverTools.resetNewMenuMusic();
	...
	function callTextbox()
	{
		...
		if (sys.FileSystem.exists(dialogPath))
		{
			...
			switch (song_){
			case "hill" | "melancholyc-tentation":
					startCircle.color = 0xFFe8828c;
			default:
