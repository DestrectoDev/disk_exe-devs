 ...
 */
class Alphabet Descarted extends FlxTypedSpriteGroup<AlphaCharacter>
{
...
class AlphaCharacter extends FlxSprite
{
	...
	public function displacementFormula() {
		...
		var formula:Float = Math.sin(Math.PI * (elapsedTotal + ((number * elapsedAverage) * 54))) * ((FlxG.elapsed / (1 / 110)) / 16);
		prevY += y;
		y = prevY + formula;
		prevY -= y + formula;
	}
        ...
        if (elapsed > 0){
			...
			// unown chance
				if (curFrame % Math.floor(frameDivisor * (Init.trueSettings.get('Framerate Cap') / 60)) == 0)
				{
					if (chances > 1)
					{
						createBold(alphabet.charAt(FlxG.random.int(0, alphabet.length - 1)));
