...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	var santa:FNFSprite;
	public var redShade:FNFSprite;
	var bgGirls:BackgroundGirls;
    
    var sineEffect:Float = 0;
	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
            ...
            case "green_hill":
				...
				sky.loadGraphic(Paths.image("stages/" + curStage + "/red_sky"));
				sky.scrollFactor.set(1.2, 1.2);
				sky.antialiasing = true;
				add(sky);

				var redlake = new FNFSprite(-636, -491);
				redlake.loadGraphic(Paths.image("stages/" + curStage + "/red_lake"));
				redlake.scrollFactor.set(1.3, 1.3);
				redlake.antialiasing = true;
				add(redlake);
				
				var floor = new FNFSprite(-636, -411);
				floor.loadGraphic(Paths.image("stages/green_hill/floor"));
				floor.scrollFactor.set(0.97, 0.95);
				floor.antialiasing = true;
				floor.scale.set(1.3, 1);
				add(floor);

				var palms = new FNFSprite(-636, -761);
				palms.loadGraphic(Paths.image("stages/green_hill/palms"));
				palms.scrollFactor.set(0.97, 0.95);
				palms.antialiasing = true;
				add(palms);

				redShade = new FNFSprite(-915.3, -1189.7);
				redShade.loadGraphic(Paths.image("stages/green_hill/redShade"));
				redShade.alpha = 0.96;
	...
	public function stageUpdateConstant(elapsed:Float, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		{
			case "green_hills":
				sineEffect += 180 * (elapsed / 4);
				redShade.alpha = 0.6 - Math.sin((Math.PI * sineEffect) / 80);
			case 'philly':
				if (trainMoving)
				{
